...
predicate targetApi(API::Node api, string qn) {
  ...
  api = API::moduleImport("structlog") and qn = "structlog.*" or
  api = API::moduleImport("google").getMember("cloud").getMember("logging").getMember("Client") and qn = "google.cloud.logging.Client" or
  api = API::moduleImport("google").getMember("cloud").getMember("logging").getMember("Client").getAnInstance().getMember("setup_logging") and qn = "google.cloud.logging.Client.setup_logging" or
  api = API::moduleImport("google").getMember("cloud").getMember("logging").getMember("Client").getAnInstance().getMember("logger") and qn = "google.cloud.logging.Client.logger" or
  api = API::moduleImport("google").getMember("cloud").getMember("logging").getMember("Logger").getAnInstance().getMember("log_text") and qn = "google.cloud.logging.Logger.log_text" or
  api = API::moduleImport("google").getMember("cloud").getMember("logging").getMember("handlers").getMember("CloudLoggingHandler") and qn = "google.cloud.logging.handlers.CloudLoggingHandler" or
  api = API::moduleImport("structlog").getAMember() and qn = "structlog.*" or
  api = API::moduleImport("structlog").getMember("contextvars").getMember("bind_contextvars") and qn = "structlog.contextvars.bind_contextvars" or
  api = API::builtin("str").getAnInstance().getMember("replace") and qn = "str.replace"
}
